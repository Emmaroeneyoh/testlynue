const Rent = require('../../modal/post_rent_modal/Rent');
...
const createProduct = async (req, res) => {
  ...
  try {
    ...
    if (req.files.length <= 3) {
      return res.status(400).send('please add atleast 5 images');
    } 
   ...
   return res.json(saveproproperty);
  } catch (err) {
    //  err ? res.status(400).send("error occured try again") : null
  }
};
...
const RentProperty = async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).send('property not found');
    console.log(user);
    res.json(user);
  } catch (error) {
    console.log(error);
...
const deleteRent = async (req, res) => {
  try {
    const { id } = req.params;
    const blog = await Rent.findById(id);
    if (!blog) return res.status(400).send('property doesnt exist');
